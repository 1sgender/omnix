...
2. **Холодный старт ~1.3 с после каждого `start()`**: после `reset()` первые
   кадры не детектят, пока не накопятся 16 эмбеддингов (вместо прогрева
   случайным шумом в эталоне). Сессии тёплые (не перечитываются), поэтому
   цена — ровно это слепое окно на каждом старте (вход в STANDBY и вход в
   AI_THINKING); в фазе раздумий окно перекрыто реальными задержками AI.
...
### Холодный старт (п.3) — реализовано
Два слоя.

1. **Тёплые сессии (харденинг 2026-09-26):** ONNX-сессии и пайплайн живут
   на уровне движка — каждый вход в STANDBY больше не перечитывает
   ~3.7 МБ ассетов и не создаёт три OrtSession заново.
2. **Движок живёт в AI_THINKING (барридж-ин):** единственный безопасный
   слот (микрофон свободен, OMNIX молчит; в `TTS_SPEAKING` запуск
   невозможен — self-trigger) занят: оркестратор запускает движок при
   входе в фазу раздумий (обе точки: `processUserQuery` и повтор после
   cloud-consent), self-voice guard останавливает его на
   `TtsState.Speaking` — одна точка покрывает все места старта TTS,
   включая стриминг ответов (§13: `speakQueued` при режиме, который всё
   ещё AI_THINKING).
   - «Omni» в AI_THINKING = голосовой барридж-ин: in-flight запрос
     отменяется (эпоха CR-07, TTS-очередь и pending-состояния сброшены),
     wake-chime, СРАЗУ слушание новой команды — повторять wake word не
     приходится. Маршрутизация — чистая функция
     `resolveWakeDetectionAction` (JVM-тест по всем режимам).
   - Near-miss запись (данные v0.2) теперь покрывает и фазу раздумий —
     акустические контексты записываемых хвостов шире.
   - Слепое окно 1.3 с остаётся на каждом `start()` (16 эмбеддингов
     контекста); в фазе раздумий оно перекрыто реальными задержками AI.
