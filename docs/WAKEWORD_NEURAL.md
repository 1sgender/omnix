...
Day-one реализация голосовой активации «Hey Jarvis»: предобученная модель
openWakeWord v0.5.1, собственный тонкий адаптер поверх ONNX Runtime,
без STT-верификации (детект подтверждает нейросеть, не распознанный текст).
...
- `training/` — фаза 2: спецификация и проверка кастомной модели «Jarvis».
...
4. **Одна модель**: «Hey Jarvis» v0.5.1. Кастомный «Jarvis» — фаза 2 (`training/`).
...
2. Набрать ≥50 произнесений «Hey Jarvis» (3 диктора, тихо/шум/улица)
