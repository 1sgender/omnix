package com.omnix.assistant.voice.service
...
import com.omnix.assistant.presentation.MainActivity
...
class OmnixVoiceService : Service() {
    ...
    companion object {
        ...
        const val CHANNEL_NAME = "OMNIX Voice Service"
        const val NOTIFICATION_ID = 1001

        private const val TAG = "OmnixVoiceService"
        private const val WAKELOCK_TIMEOUT_MS = 8 * 60 * 60 * 1000L

        const val ACTION_START = "com.omnix.action.START_SERVICE"
        const val ACTION_STOP = "com.omnix.action.STOP_SERVICE"
        const val ACTION_PAUSE = "com.omnix.action.PAUSE_SERVICE"
        const val ACTION_RESUME = "com.omnix.action.RESUME_SERVICE"
        ...
        fun start(context: Context): Boolean {
            ...
            val intent = Intent(context, OmnixVoiceService::class.java).apply {
                action = ACTION_START
            }
            return try {
    ...
    private inner class OmnixTelephonyCallback : TelephonyCallback(), TelephonyCallback.CallStateListener {
    ...
    override fun onCreate() {
        ...
        if (!safeStartForeground(
                getString(R.string.omnix_slushaet),
                "onCreate"
            )
        ) {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent?.action) {
            ...
            ACTION_PAUSE -> {
                ...
                updateNotification(getString(R.string.omnix_na_pauze))
            }
            ACTION_STOP -> {
                orchestrator.stopServicePipeline()
        ...
        serviceScope.launch {
            orchestrator.currentMode.collectLatest { mode ->
                val statusText = when (mode) {
                    OrchestratorMode.STANDBY_WAKE_WORD -> getString(R.string.omnix_slushaet_v_naushnike)
    ...
    private fun buildNotification(statusText: String): Notification {
        ...
        val stopIntent = Intent(this, OmnixVoiceService::class.java).apply { action = ACTION_STOP }
        val pendingStop = PendingIntent.getService(
            this, 2, stopIntent, PendingIntent.FLAG_IMMUTABLE
        )

        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("OMNIX Voice Service")
