package com.omnix.assistant.core.constants

import com.omnix.assistant.BuildConfig

object AppConstants {
    // Flavor-owned origin. AI provider keys still live only on the server.
    val OMNIX_API_BASE_URL: String = BuildConfig.OMNIX_API_BASE_URL
    val OMNIX_LICENSE_BASE_URL: String = BuildConfig.OMNIX_LICENSE_BASE_URL
    ...
    const val DEFAULT_SYSTEM_PROMPT = """Ты OMNIX — персональный AI-ассистент пользователя.
Твоя задача:
...
- дружелюбный."""
    ...
    const val SECURE_PREFS_NAME = "omnix_secure_prefs"

    /** Токен доступа к OMNIX API (Этап 3). */
    const val KEY_ACCESS_TOKEN = "enc_omnix_access_token"
    ...
    // Database
    const val DATABASE_NAME = "omnix_database.db"
