...
interface LicenseManager {
     ...
     * Сервер сверяет его с привязкой omx_-токена на enforcement-пути
     * (заголовок X-Omnix-Device); клиент — НЕ источник истины, сервер
...
class LicenseManagerImpl @Inject constructor(
    ...
    companion object {
        ...
        private const val PREFS_NAME = "omnix_hardware_license"
            ...
            is LicenseCodeValidator.CodeVerdict.BoxCodeValid -> {
                ...
                return try {
                    ...
                    ActivationResult.Success(
                        licenseInfo = info,
                        message = context.getString(R.string.omnix_uspeshno_aktivirovan)
                    )
                } catch (t: Throwable) {
    ...
    private fun getDeviceHardwareId(): String {
        ...
        val generated = try {
            ...
            if (androidId != null) {
                ...
                "OMX-$manufacturer-$model-${androidId.takeLast(12).uppercase(Locale.ROOT)}"
            } else {
    ...
    private fun newFallbackHardwareId(): String =
        "OMX-DEVICE-${UUID.randomUUID().toString().take(12).uppercase(Locale.ROOT)}"
}
