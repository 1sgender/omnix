 ...
 * к OMNIX API. Ключи Groq/Gemini/OpenRouter на устройстве больше не хранятся
 * и не используются — они живут исключительно на сервере.
 */
interface SecurityManager {

    /** Токен доступа к OMNIX API (Bearer). */
...
class SecurityManagerImpl @Inject constructor(
    ...
    override fun saveAccessToken(token: String) {
        ...
        require(AccessTokenPolicy.isValid(trimmed)) {
            "OMNIX access token must be ${AccessTokenPolicy.MIN_LENGTH}..${AccessTokenPolicy.MAX_LENGTH} characters without whitespace"
        }
