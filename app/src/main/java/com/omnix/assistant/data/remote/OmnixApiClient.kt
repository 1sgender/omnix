...
/**
 * Клиент OMNIX API (Этап 3).
 *
 * ```
 * Android CloudAi → OmnixApiClient → OMNIX API → AI Router → Provider Manager
 * ```
 *
...
@Serializable
data class OmnixApiErrorResponseDto(
    @SerialName("success") val success: Boolean = false,
    @SerialName("error") val error: OmnixApiErrorDto = OmnixApiErrorDto()
)

/** Ошибка OMNIX API с machine-readable кодом. */
class OmnixApiException(
    val code: String,
...
class OmnixApiClient @Inject constructor(
    ...
    companion object {
        ...
        /**
         * Базовый URL OMNIX API. Тот же хост использует LicenseServerValidator.
         */
        val BASE_URL: String = AppConstants.OMNIX_API_BASE_URL
    ...
    suspend fun execute(
        ...
        if (!AccessTokenPolicy.isValid(token)) {
            return@withContext Resource.Error(
                ...
                "Токен доступа OMNIX отсутствует или имеет неверный формат."
            )
        }
        ...
        return@withContext try {
            ...
            Resource.Error(e, "Сервер OMNIX вернул слишком большой ответ.")
        } catch (e: Exception) {
    ...
    private fun parseHttpResponse(response: Response, body: String, requestId: String): Resource<String> {
        if (response.isSuccessful) {
            val parsed = json.decodeFromString(OmniAiResponseDto.serializer(), body)
            ...
            return Resource.Error(
                OmnixApiException("EMPTY_RESPONSE", requestId, "Пустой ответ сервера"),
                "Сервер вернул пустой ответ."
            )
        }
        val error = runCatching {
            json.decodeFromString(OmnixApiErrorResponseDto.serializer(), body).error
        }.getOrElse {
            OmnixApiErrorDto(code = "HTTP_${response.code}", requestId = requestId)
        }
        Log.w(TAG, "ai request failed | code=${error.code} | requestId=${error.requestId}")
        return Resource.Error(
            OmnixApiException(error.code, error.requestId, error.message),
    ...
    private fun userMessageFor(code: String): String = when (code) {
        ...
        "QUOTA_EXCEEDED" -> "Дневной лимит тарифа исчерпан. Продолжим завтра, сэр."
