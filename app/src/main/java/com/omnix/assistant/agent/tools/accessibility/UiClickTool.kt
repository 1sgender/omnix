    ...
    override suspend fun execute(arguments: JsonObject): ToolExecutionResult {
        ...
        if (!OmnixAccessibilityService.isServiceRunning()) {
            ...
            return ToolExecutionResult.userActionRequired(
                summary = if (opened) {
                    "Для управления интерфейсом необходимо включить OMNIX Accessibility Service. Открыл настройки специальных возможностей."
                } else {
                    "Для управления интерфейсом необходимо включить OMNIX Accessibility Service в настройках."
                },
        ...
        return try {
            val performed: AccessibilityActionResult = when (action) {
                "click" -> OmnixAccessibilityService.clickByText(targetText)
                "scroll_down" -> OmnixAccessibilityService.scrollDown()
                "scroll_up" -> OmnixAccessibilityService.scrollUp()
