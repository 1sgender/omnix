...
class SetBrightnessTool @Inject constructor(
    ...
    override val capability: OmniCapability = OmniCapability.Brightness
    ...
    override suspend fun execute(arguments: JsonObject): ToolExecutionResult {
        ...
        if (!capabilities.canWriteSystemSettings()) {
            ...
            return ToolExecutionResult.userActionRequired(
                summary = if (opened) {
                    "Чтобы менять яркость, OMNIX нужно разрешение «Изменение системных настроек». Открыл нужный экран — включите доступ, сэр."
                } else {
