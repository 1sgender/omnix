package com.omnix.assistant.agent.core
...
interface OmniTool {
    ...
    /**
     * True when arguments can leave the OMNIX process/device boundary, even if
 ...
 * устройстве» ДО вызова [OmniTool.execute] — вместо того, чтобы узнавать об
 * ограничении Android постфактум.
 */
interface CapabilityAwareTool : OmniTool {
    val capabilityContract: ToolCapabilityContract

    /**
     * Группа Android Capability Layer, к которой относится инструмент
     * (например, [OmniCapability.Bluetooth] для device.bluetooth).
