package com.omnix.assistant.agent.engine
...
import com.omnix.assistant.agent.observation.NextActionHint
import com.omnix.assistant.agent.observation.Observation
import com.omnix.assistant.agent.planner.*
...
class AgentCognitiveLoop @Inject constructor(
    ...
    companion object {
         ...
         * что отдельные tool-вызовы уже имеют собственный [OmniTool.executionTimeoutMs]
    ...
    private suspend fun runPlanInternal(initialPlan: ExecutionPlan): PlanExecutionSummary {
        ...
        while (currentStepIdx < currentPlan.steps.size) {
            ...
            if (observation.success) {
                ...
                if (verified) {
                    if (com.omnix.assistant.agent.tools.accessibility.ScreenContentPrivacy.isScreenReaderCall(step.toolCall.toolId)) {
