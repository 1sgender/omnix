 ...
 * то, что раньше делал [com.omnix.assistant.agent.pipeline.AgentPipeline],
 * но за единым контрактом [ExecutionRequest] / [ExecutionResult].
 */

/**
 * CLOUD AI = OMNIX API (Этап 3).
 *
 * ```
 * CloudAiExecutor → AIRepository → OmnixApiAiClient → OMNIX API
...
class RepositoryCloudAiExecutor @Inject constructor(
    ...
    override suspend fun complete(request: ExecutionRequest): Resource<String> {
        ...
        Log.d(TAG, "cloud request → OMNIX API | source=${request.source} | privacy=${request.effectivePrivacyLevel}")
