package com.omnix.assistant.agent.capability
 ...
 * «может ли OMNIX вообще что-то сделать в этом домене на данном устройстве»,
 * а лист — «может ли OMNIX выполнить конкретное действие».
 *
 * ```
 * OmniCapability
 ...
 */
sealed interface OmniCapability {
    ...
    data object Bluetooth : OmniCapability {
    ...
    data object Wifi : OmniCapability {
    ...
    data object Brightness : OmniCapability {
    ...
    data object Screenshot : OmniCapability {
    ...
    data object Apps : OmniCapability {
    ...
    data object Sms : OmniCapability {
    ...
    data object Call : OmniCapability {
    ...
    data object Media : OmniCapability {
    ...
    data object Accessibility : OmniCapability {
        override val id: String = "accessibility"
        override val description: String = "Служба специальных возможностей OMNIX (чтение экрана, UI-клики)"
        override val leaves: Set<DeviceCapability> = setOf(
            DeviceCapability.USE_ACCESSIBILITY_SERVICE
        )
    }

    data object Location : OmniCapability {
    ...
    companion object {
        ...
        val all: List<OmniCapability> = listOf(
            Bluetooth, Wifi, Brightness, Screenshot, Apps,
            Sms, Call, Media, Accessibility, Location
        )

        /** Разрешение группы по dot-идентификатору. */
        fun byId(id: String): OmniCapability? = all.firstOrNull { it.id == id }
    }
}

/**
 * Агрегирует статусы листовых проверок в статус группы [OmniCapability].
 *
