    ...
    override suspend fun execute(arguments: JsonObject): ToolExecutionResult {
        ...
        return try {
            if (action == "copy" || text.isNotEmpty()) {
                val clip = ClipData.newPlainText("OMNIX", text)
                ...
                if (readBack == text) {
                    ...
                    ToolExecutionResult.failure(
                        "Не удалось подтвердить копирование: система ограничила доступ к буферу обмена. Откройте OMNIX на экране и повторите",
