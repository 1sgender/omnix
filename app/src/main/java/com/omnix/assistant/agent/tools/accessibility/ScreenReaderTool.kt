    ...
    override suspend fun execute(arguments: JsonObject): ToolExecutionResult {
        ...
        if (!OmnixAccessibilityService.isServiceRunning()) {
            ...
            return ToolExecutionResult.userActionRequired(
                summary = if (opened) {
                    "Для чтения экрана необходимо включить OMNIX Accessibility Service. Открыл настройки специальных возможностей."
                } else {
                    "Для чтения экрана необходимо включить OMNIX Accessibility Service в настройках."
                },
        ...
        return try {
            when (val readResult = OmnixAccessibilityService.getScreenContent()) {
