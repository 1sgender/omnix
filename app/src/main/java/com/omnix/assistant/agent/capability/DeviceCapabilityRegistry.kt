...
import com.omnix.assistant.agent.tools.accessibility.OmnixAccessibilityService
import dagger.hilt.android.qualifiers.ApplicationContext
import javax.inject.Inject
import javax.inject.Singleton

/**
 * Единая точка правды о том, что OMNIX реально может сделать на этом устройстве.
 *
...
class DeviceCapabilityRegistry @Inject constructor(
    ...
    override fun statusOf(capability: DeviceCapability): CapabilityStatus = when (capability) {
        ...
        DeviceCapability.TAKE_SCREENSHOT_ACCESSIBILITY -> when {
            ...
            !OmnixAccessibilityService.isServiceRunning() -> CapabilityStatus.UserActionRequired(
                "Служба специальных возможностей OMNIX не включена"
            )
            else -> CapabilityStatus.Available
        }
        ...
        DeviceCapability.USE_ACCESSIBILITY_SERVICE ->
            if (OmnixAccessibilityService.isServiceRunning()) CapabilityStatus.Available
            else CapabilityStatus.UserActionRequired("Служба специальных возможностей OMNIX не включена")
    }

    /**
     * Статус группы Android Capability Layer: агрегация листовых проверок.
     */
    override fun statusOf(capability: OmniCapability): CapabilityStatus =
     ...
     */
    fun snapshotByGroup(): Map<OmniCapability, CapabilityStatus> =
        OmniCapability.all.associateWith { statusOf(it) }
}
