package com.omnix.assistant.agent.safety
...
import com.omnix.assistant.agent.policy.ActionPolicyEngine
import com.omnix.assistant.agent.policy.PolicyDecision
import com.omnix.assistant.agent.policy.ProposedAction
...
class ToolPermissionManager @Inject constructor(
    ...
    private val policyEngine: ActionPolicyEngine = ActionPolicyEngine(
        com.omnix.assistant.agent.policy.DefaultActionPolicySettingsProvider()
    )
) {
     ...
     */
    fun isExecutionAllowed(tool: OmniTool): Boolean = !requiresConfirmation(tool)

    private fun requiresConfirmation(tool: OmniTool): Boolean {
