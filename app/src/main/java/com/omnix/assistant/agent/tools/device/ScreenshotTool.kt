package com.omnix.assistant.agent.tools.device
...
class ScreenshotTool @Inject constructor(
    ...
    override val capability: OmniCapability = OmniCapability.Screenshot
    ...
    override suspend fun execute(arguments: JsonObject): ToolExecutionResult {
        ...
        when (val status = capabilities.statusOf(DeviceCapability.TAKE_SCREENSHOT_ACCESSIBILITY)) {
            is CapabilityStatus.UserActionRequired -> {
                ...
                return ToolExecutionResult.userActionRequired(
                    summary = if (opened) {
                        "Для снимков экрана нужно включить службу OMNIX в разделе «Специальные возможности». Открыл нужный экран, сэр."
                    } else {
                        "Для снимков экрана включите службу OMNIX в разделе «Специальные возможности», сэр."
                    },
        ...
        val taken = OmnixAccessibilityService.takeSystemScreenshot()
        return if (taken) {
