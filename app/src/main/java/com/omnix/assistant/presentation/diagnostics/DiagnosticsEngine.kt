...
import com.omnix.assistant.core.license.ServerLicenseValidationResult
import com.omnix.assistant.core.network.NetworkMonitor
import com.omnix.assistant.presentation.state.ClipRepository
import com.omnix.assistant.voice.audio.BluetoothAudioRouter
import com.omnix.assistant.voice.audio.BluetoothAudioState
...
class DiagnosticsEngine @Inject constructor(
    ...
    private fun checkAccessibility(): Pair<DiagnosticStatus, String> {
        val expected = ComponentName(context, OmnixAccessibilityService::class.java)
            .flattenToString()
        ...
        return if (enabled.any { it.equals(expected, ignoreCase = true) }) {
            DiagnosticStatus.OK to "OmnixAccessibilityService enabled"
        } else {
