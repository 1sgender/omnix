...
import com.omnix.assistant.data.remote.MessageDto
...
/**
 * H-02 / Refactor #3: регрессионные тесты для [OmnixApiAiClient] после удаления
 ...
 */
class OmnixApiAiClientPrivacyMatrixTest {

    private lateinit var network: OmnixApiClient
    ...
    fun `NORMAL proceeds to network without consent`() = runBlocking {
        val ai = OmnixApiAiClient(network)
    ...
    fun `baseline complete() uses NORMAL level and CHAT source (legacy translator path)`() = runBlocking {
        val net = mockk<OmnixApiClient>(relaxed = false)
        coEvery {
            net.execute(any(), any(), any(), any(),
                systemContext = any(), cloudExplicitlyAllowed = any(), history = any(), requestId = any())
        } returns Resource.Success("ok")
        val ai = OmnixApiAiClient(net)
