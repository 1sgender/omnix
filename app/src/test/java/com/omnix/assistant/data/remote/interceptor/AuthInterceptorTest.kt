...
/**
 * V007 device binding: the server rejects omx_ license tokens on the AI path
 * fail-closed when X-Omnix-Device is missing. [OmnixApiClient] sets
...
class AuthInterceptorTest {
    ...
    private val licenses = FakeLicenseManager("OMX-TEST-DEVICE-01")
    ...
    fun `device header is added when caller already set Authorization`() {
        val request = Request.Builder()
            .url(OmnixApiClient.BASE_URL + OmnixApiClient.EXECUTE_PATH)
        ...
        assertEquals("OMX-TEST-DEVICE-01", sent.header("X-Omnix-Device"))
    }

    @Test
    fun `token and device header are added when both are missing`() {
        val request = Request.Builder()
            .url(OmnixApiClient.BASE_URL + "/v1/license/validate")
            .post("".toRequestBody())
            .build()
    ...
    fun `explicit device header is never overwritten`() {
        val request = Request.Builder()
            .url(OmnixApiClient.BASE_URL + OmnixApiClient.EXECUTE_PATH)
            .header("Authorization", "Bearer caller-token")
            .header("X-Omnix-Device", "CALLER-DEVICE")
            .post("".toRequestBody())
            .build()

        val sent = intercept(request)

        assertEquals("CALLER-DEVICE", sent.header("X-Omnix-Device"))
    }
    ...
    fun `blank device id sends no device header but keeps Authorization`() {
        ...
        assertNull(sent.header("X-Omnix-Device"))
        assertEquals("Bearer caller-token", sent.header("Authorization"))
    }
    ...
    fun `Authorization to a foreign host is refused`() {
        ...
        try {
            ...
            assertTrue(e.message.orEmpty().contains("outside the configured OMNIX origin"))
        }
    }
