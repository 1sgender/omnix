...
class LicenseCodeValidatorTest {
    ...
    private val hardwareId = "OMX-TEST-DEVICE-ABCDEF"
    ...
    private fun record(days: Long, token: String? = "omx_" + "x".repeat(43)) = ServerLicenseRecord(
        accessToken = token,
        planId = "earclip-monthly",
        productId = "omnix-clip",
        startsAt = start,
    ...
    fun `server redemption preserves authoritative entitlement`() {
        ...
        assertEquals(
            ...
            redeem("OMX-ABCDE-FGHJK-LMNPQ-RSTUV", ServerRedemptionResult.Success(license))
        )
    }
    ...
    fun `server rejection and outage fail closed`() {
        assertEquals(
            ...
            redeem("OMX-ABCDE-FGHJK-LMNPQ-RSTUV", ServerRedemptionResult.NotRedeemable)
        )
        assertEquals(
            LicenseCodeValidator.CodeVerdict.ServiceUnavailable,
            redeem("OMX-ABCDE-FGHJK-LMNPQ-RSTUV", ServerRedemptionResult.ServiceUnavailable)
        )
        assertEquals(
            LicenseCodeValidator.CodeVerdict.RateLimited,
            redeem("OMX-ABCDE-FGHJK-LMNPQ-RSTUV", ServerRedemptionResult.RateLimited)
        )
    }
    ...
    fun `canonical code and hardware id reach server`() = runBlocking {
        val fake = FakeServerValidator(ServerRedemptionResult.Success(record(30)))
        val code = "OMX-ABCDE-FGHJK-LMNPQ-RSTUV"
