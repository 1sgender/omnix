...
class OmniCapabilityTest {
    ...
    fun `layer exposes exactly the ten android capability groups in spec order`() {
        assertEquals(
            ...
            OmniCapability.all.map { it.id }
        )
    }

    @Test
    fun `group ids are unique`() {
        val ids = OmniCapability.all.map { it.id }
        assertEquals(ids.size, ids.distinct().size)
    }

    @Test
    fun `byId resolves every group and unknown ids resolve to null`() {
        OmniCapability.all.forEach { group ->
            assertSame(group, OmniCapability.byId(group.id))
        }
        assertNull(OmniCapability.byId("device.unknown"))
        assertNull(OmniCapability.byId(""))
    }

    @Test
    fun `every leaf capability belongs to exactly one group`() {
        val assignments = OmniCapability.all.flatMap { group ->
    ...
    fun `fake registry aggregates group status over leaves`() {
        ...
        assertTrue(fake.statusOf(OmniCapability.Accessibility) is CapabilityStatus.UserActionRequired)

        // Разрешили службу на уровне листа — группа становится Available.
        val fakeOk = FakeCapabilityRegistry.create()
        assertTrue(fakeOk.statusOf(OmniCapability.Accessibility) is CapabilityStatus.Available)
    }
    ...
    fun `bluetooth group aggregation reflects android 13 restriction`() {
        ...
        assertTrue(fake.statusOf(OmniCapability.Bluetooth) is CapabilityStatus.Available)
    }
}
