package com.omnix.assistant.agent.tools
...
import com.omnix.assistant.agent.safety.PreflightVerdict
import com.omnix.assistant.agent.safety.ToolPermissionManager
import com.omnix.assistant.agent.tools.device.BluetoothTool
import com.omnix.assistant.agent.tools.device.OpenAppTool
import com.omnix.assistant.agent.tools.device.SetVolumeTool
import com.omnix.assistant.agent.tools.device.WifiTool
import com.omnix.assistant.agent.tools.productivity.AlarmTimerTool
import com.omnix.assistant.agent.apps.AppResolver
...
class ToolContractConformanceTest {
    ...
    private fun tier1Tools(context: Context, capabilities: DeviceCapabilityRegistry): List<OmniTool> = listOf(
    ...
    fun `plain tool with ungranted declared permissions is blocked by preflight`() {
        val checker = mockk<com.omnix.assistant.agent.capability.CapabilityChecker>()
        ...
        val verdict = manager.preflight(tool, com.omnix.assistant.agent.model.ToolCall("plain.tool", kotlinx.serialization.json.JsonObject(emptyMap())))
    ...
    private class PlainDeclaredTool(
        ...
        override val category = com.omnix.assistant.agent.core.ToolCategory.SYSTEM
        override val parametersSchema = kotlinx.serialization.json.JsonObject(emptyMap())
        override val riskLevel = com.omnix.assistant.agent.model.ToolRisk.SAFE
