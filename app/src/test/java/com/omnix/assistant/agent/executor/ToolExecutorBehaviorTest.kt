package com.omnix.assistant.agent.executor
...
import com.omnix.assistant.agent.model.ToolExecutionStatus
...
class ToolExecutorBehaviorTest {
    ...
    private fun executor(vararg tools: OmniTool): ToolExecutor {
    ...
    fun `automation origin forces confirmation for messaging even when user policy allows`() = runBlocking {
        ...
        val permissive = com.omnix.assistant.agent.policy.ActionPolicySettings(
            messagingPolicy = com.omnix.assistant.agent.policy.MessagingConfirmationPolicy.NEVER
        )
        val provider = object : com.omnix.assistant.agent.policy.ActionPolicySettingsProvider {
            private val state = kotlinx.coroutines.flow.MutableStateFlow(permissive)
            override val settings: kotlinx.coroutines.flow.StateFlow<com.omnix.assistant.agent.policy.ActionPolicySettings> = state
            override fun current() = state.value
            override suspend fun update(transform: (com.omnix.assistant.agent.policy.ActionPolicySettings) -> com.omnix.assistant.agent.policy.ActionPolicySettings) {
                state.value = transform(state.value)
            }
        }
        val manager = ToolPermissionManager(
            FakeCapabilityRegistry.create(),
            com.omnix.assistant.agent.policy.ActionPolicyEngine(provider)
        )
        ...
        val direct = executor.execute(smsCall, com.omnix.assistant.agent.policy.ActionOrigin.USER_REQUEST)
        assertEquals(ToolExecutionStatus.SUCCESS, direct.status)

        // Автоматизация с теми же настройками — обязательное подтверждение.
        val automated = executor.execute(smsCall, com.omnix.assistant.agent.policy.ActionOrigin.AUTOMATION)
