...
### Changed
- **Короткий код активации с карточки Clip — бэкенд догоняет мок (решение владельца «продолжай», длина 6 по моку).** Новые коды выпускаются короткими: `generateBoxCode()` — 6 символов того же неом ambiguity-алфавита (32⁶ ≈ 1.07 млрд комбинаций + rate limiting + HMAC-хранение; длина — константа `BOX_CODE_LENGTH`, смена на 8 = две правки: сервер + ячейки). `normalizeLicenseCode` принимает оба формата — длинные OMX-/JRV- остаются погашаемыми (прецедент JRV после ребрендинга); регистр, пробелы и дефисы прощаются. Клиентский `MIN_CODE_LENGTH` 8→6. Ячейки приложения: буквы+цифры, текстовая клавиатура с автоверхним регистром (письмо дизайнера: «цифр или букв»; отклонение от inputMode=numeric демо). codeHint короткого кода — 3 символа вместо 5 (не раскрывать почти весь код). Тесты: +7 серверных (LicenseCryptoTest), ячеечные обновлены. **НЕ развернуто:** работающий staging-сервер продолжает выпускать длинные коды до деплоя владельцем; непогашенные длинные коды не введутся в 6 ячеек — перевыпустить короткими после деплоя.
...
### Changed
- **Экран «Диалог» по моку владельца (2026-09-26): пузыри с обеих сторон и однострочный ввод.** Реплика пользователя — светлый пузырь справа, ответ OMNIX — тёмно-серый пузырь слева (новые токены `bubbleUser`/`onBubbleUser`/`bubbleAi`/`onBubbleAi`, цвета дословно из мока; светлая схема — пузырь пользователя системно-синий, как в моке). Подписи «Вы»/«OMNIX» стоят вплотную над пузырём, одним блоком (4 dp, та же сторона), вся реплика — один узел TalkBack («Вы: …»). Вес шрифта — Normal везде, кроме заголовка экрана: заголовок снижен до `heading` (20 sp/600), текст реплик — `subheadline` (15 sp), «кричащая» иерархия убрана. Ошибка сети больше не выглядит как обычный ответ ассистента: новая `MessageRole.ERROR` (строка в Room — миграция не нужна) + приглушённый красноватый пузырь (токен `errorBubble`) с иконкой-предупреждением (`OmnixAlertIcon`) и обычным, не жирным текстом; сетевой сбой получает нейтральную формулировку из мока («Ошибка сети. Проверьте соединение и повторите»), «сырой» текст исключения в чат больше не уходит; диагностические части сохранены — хинт про нескачанную офлайн-модель дописывается к сетевой ошибке, несетевые сбои хранят свой текст. Точки «думает» теперь внутри пузыря — «печатает», а не отдельный виджет. Поле ввода — одна строка: капсула-поле + отдельные круглые кнопки микрофона и отправки (видимый круг 32 dp, тактильная зона 44 dp §57, отправка — залитая), плейсхолдер больше не переносится на два ряда; над полем — hairline как в моке. Таб-бар: центр — «Главная» со знаком-атомом OMNIX (`OmnixAtomIcon`) вместо вордмарки «OMNIX» — дубль с подписями реплик ушёл; выделение активного пункта (жирный, светлый) сохранено. JVM-тесты: `MessageRoleTest` (контракт персистентности новой роли) и `SendPromptUseCaseErrorTest` (роль и текст сохранённых ошибок).

### Changed
- **Wake word: харденинг по ревью владельца (2026-09-26) — пин моделей, шум-адаптивный порог, мёртвый микрофон, near-miss для v0.2.** (1) Пиннинг ONNX-моделей по sha256 в коде (`ModelDigests`): подмена/повреждение ассета ловится ДО загрузки в ORT и падает громкой `ModelCorrupted` (ожидание+факт дайджеста) — CI держит пин в синхроне с файлами. (2) Динамический порог: RMS каждого 80-мс кадра, шумовой пол — 20-й перцентиль окна 2 с (не гоняется за речью), буст до +0.10 при SNR 0 дБ, в тишине порог базовый (0.35) — recall не страдает; выключается `noiseAdaptiveThreshold=false`. (3) «Мёртвый» микрофон: 20 с константного потока (размах ≤ 4 LSB; живая тишина с ADC-джиттером не детектится) → `MicrophoneDeadSignal` вместо молчаливой работы вслепую. (4) Потеря клипа/наушников сбрасывает недодетектированную patience-серию явно (`resetDetectionSeries`) — «Omni» из старой акустики не подтвердится в новой. (5) Метрики: к средним задержкам mel/embedding/classifier добавлен P95 (всплески больше не маскируются), debug-лог — с разбором стадий и шумовым полом (формат для 06-скрипта сохранён). (6) Near-miss захват для данных v0.2 (флаг `nearMissCapture`, дефолт ВЫКЛ): WAV-хвост 2.5 с на кадры со скором ≥0.25, не ставшие детекцией — реальные негативы retrain'а; только app-private, кап 40 МБ, манифест без метаданных пользователя, дебаунс 30 с. (7) Сессии ONNX и пайплайн переехали на уровень движка: каждый вход в STANDBY больше не перечитывает 3.7 МБ ассетов и не создаёт три OrtSession заново (инфраструктура под закрывание 1.3-с холодного старта — само закрытие требует запуска движка в фазе AI_THINKING и идёт отдельной задачей). Чистая логика — JVM-тесты: `ModelDigestsTest`, `NoiseAdaptiveThresholdTest`, `DeadSignalDetectorTest`, `NearMissRecorderTest` + расширения `DetectionPolicyTest`/`WakeWordMetricsTest`.

### Changed
- **Настройки разработчика: явное включение near-miss записи (модель v0.2) в dev/staging.** (1) Новый переключатель «Запись near-miss» в разделе «Для разработчика»: включает захват 2.5-с аудио-хвостов на кадры, где «Omni» почти распознано (скор ≥0.25, детекции нет) — данные для обучения модели v0.2 на реальных голосах (протокол — training/COLLECTING.md §5). Только явное включение, запись строго локальная (app-private, кап 40 МБ, без персональных данных), кнопка «Удалить записи» — немедленный privacy-выход. (2) Flavor-гейт: механизм записи существует только в dev/staging-сборках; в prod он не существует ни при каком состоянии настроек (BuildConfig-флаг + гейт в DataStore — защита в глубину). (3) Чистая функция гейта `resolveNearMissCapture` — JVM-тест.

### Added
- **Барридж-ин: «Omni» теперь работает, пока OMNIX «думает» (wake-движок живёт в фазе AI_THINKING).** До этого движок запускался только в STANDBY — на время запроса (десятки секунд на сетевом AI) остановить его голосом было нельзя. Теперь при входе в фазу раздумий оркестратор запускает движок (тёплая ONNX-сессия из харденинга — без перечитывания ~3.7 МБ моделей), а self-voice guard гасит его на `TtsState.Speaking`: одна точка покрывает ВСЕ места старта TTS, включая стриминг ответов (§13, `speakQueued` при неизменном режиме AI_THINKING), — собственная речь OMNIX не может сработать wake word. «Omni» во время раздумий = отмена in-flight запроса (эпоха CR-07, сброс TTS-очереди и pending-состояний) + wake-chime + СРАЗУ слушание новой команды — повторять wake word не приходится. Маршрутизация детекта — чистая функция `resolveWakeDetectionAction` (JVM-тест по всем 10 режимам). Это и закрывает «холодный старт» п.3 ревью владельца (docs/WAKEWORD_NEURAL.md, статус обновлён): 1.3-с слепое окно после каждого старта осталось (16 эмбеддингов контекста), но в фазе раздумий оно перекрыто реальными задержками AI. Побочный эффект на плюс: near-miss запись (данные v0.2) теперь покрывает и фазу раздумий — шире акустические контексты записываемых хвостов.
