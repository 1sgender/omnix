...
data class AiGenerationConfig(
    ...
    companion object {
        const val DEFAULT_SYSTEM_PROMPT =
            "Ты OMNIX — персональный голосовой AI-ассистент. Отвечай кратко и по существу: " +
...
data class ServerConfig(
    ...
    companion object {
        ...
        fun fromEnv(env: (String) -> String? = System::getenv): ServerConfig {
            ...
            val tokenPairs = str("OMNIX_CLIENT_TOKENS")
                ?.split(",")
                ?.mapNotNull { entry ->
            ...
            require(tokenPairs.map { it.first }.distinct().size == tokenPairs.size) {
                "duplicate client token in OMNIX_CLIENT_TOKENS"
            }
            ...
            val licenseSubsystem = if (licenseSignals.all { it == null }) {
                ...
                val plans = plansRaw.split(';').filter { it.isNotBlank() }.map { entry ->
                    ...
                    com.omnix.server.license.BillingPlan(
                        id = fields[0].trim(),
                ...
                LicenseSubsystemConfig(
                    database = com.omnix.server.persistence.DatabaseConfig(
                        jdbcUrl = databaseUrl,
                        user = databaseUser,
                    ...
                    paddle = com.omnix.server.billing.PaddleBillingConfig(
                    ...
                    heleket = com.omnix.server.billing.HeleketBillingConfig(
