package com.omnix.server.observability
...
class Metrics {
    ...
    fun prometheus(): String = buildString {
        ...
        counter("omnix_requests_total", "Total AI execute requests received.", requestsTotal.get())
        counter("omnix_requests_success_total", "AI execute requests answered successfully.", requestsSuccess.get())
        counter("omnix_requests_failed_total", "AI execute requests that failed.", requestsFailed.get())
        counter("omnix_requests_rate_limited_total", "Requests rejected by rate limiting.", rateLimitedTotal.get())
        counter("omnix_requests_unauthorized_total", "Requests rejected by authentication.", unauthorizedTotal.get())
        counter("omnix_requests_privacy_blocked_total", "Requests blocked by the privacy classifier.", privacyBlockedTotal.get())
        counter("omnix_tokens_total", "Total tokens reported by providers.", totalTokens.get())

        val providerIds = (providerSuccess.keys + providerFailure.keys).sortedBy { it.name }
        if (providerIds.isNotEmpty()) {
            append("# HELP omnix_provider_success_total Successful provider calls.\n")
            append("# TYPE omnix_provider_success_total counter\n")
            for (id in providerIds) {
                append("omnix_provider_success_total{provider=\"${id.name}\"} ${counter(providerSuccess, id).get()}\n")
            }
            append("# HELP omnix_provider_failure_total Failed provider calls.\n")
            append("# TYPE omnix_provider_failure_total counter\n")
            for (id in providerIds) {
                append("omnix_provider_failure_total{provider=\"${id.name}\"} ${counter(providerFailure, id).get()}\n")
            }
            append("# HELP omnix_provider_latency_ms_sum Cumulative provider latency in milliseconds.\n")
            append("# TYPE omnix_provider_latency_ms_sum counter\n")
            for (id in providerIds) {
                append("omnix_provider_latency_ms_sum{provider=\"${id.name}\"} ${counter(providerLatencySum, id).get()}\n")
            }
        }

        if (failureKinds.isNotEmpty()) {
            append("# HELP omnix_provider_failure_kind_total Provider failures by kind.\n")
            append("# TYPE omnix_provider_failure_kind_total counter\n")
            for ((kind, value) in failureKinds.entries.sortedBy { it.key }) {
                append("omnix_provider_failure_kind_total{kind=\"$kind\"} ${value.get()}\n")
            }
        }
