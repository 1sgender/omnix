...
class AdminUiHandler(
    ...
    private val providerManager: com.omnix.server.provider.ProviderManager,
     ...
     */
    private fun licenseStatusParam(request: HttpRequestContext): com.omnix.server.license.LicenseStatus? {
        val raw = request.rawQuery?.split('&')
            ?.firstOrNull { it.substringBefore('=') == "status" }
            ?.substringAfter('=', "")?.takeIf { it.isNotEmpty() } ?: return null
        return runCatching {
            com.omnix.server.license.LicenseStatus.valueOf(raw.uppercase().trim())
        }.getOrNull()
    }

    private fun licenses(principal: AdminPrincipal, status: com.omnix.server.license.LicenseStatus?): String {
        val rows = queries.licenses(50, 0, status)
        // MVP-дерево Licenses: ALL | ACTIVE | EXPIRED | ... — быстрые вкладки.
        val filterLinks =
            listOf<com.omnix.server.license.LicenseStatus?>(null) + com.omnix.server.license.LicenseStatus.entries
