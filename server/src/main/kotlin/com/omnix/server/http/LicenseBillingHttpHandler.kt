...
import com.omnix.server.api.BillingCheckoutRequest
import com.omnix.server.api.BillingCheckoutResponse
import com.omnix.server.api.BillingWebhookResponse
import com.omnix.server.api.ClipBindingDto
import com.omnix.server.api.LicenseIssueRequest
import com.omnix.server.api.LicenseIssueResponse
import com.omnix.server.api.LicenseRedeemRequest
import com.omnix.server.api.LicenseRedeemResponse
import com.omnix.server.api.LicenseRevokeRequest
import com.omnix.server.api.LicenseValidateRequest
...
import com.omnix.server.auth.Permission
import com.omnix.server.billing.BillingEventApplyResult
import com.omnix.server.billing.BillingOrder
import com.omnix.server.billing.BillingProviderId
import com.omnix.server.billing.BillingService
import com.omnix.server.billing.CreateCheckoutOutcome
import com.omnix.server.billing.HeleketWebhookVerifier
import com.omnix.server.billing.PaddleWebhookVerifier
import com.omnix.server.billing.WebhookVerificationResult
...
import com.omnix.server.license.RedeemOutcome
import com.omnix.server.license.ValidationFailure
import com.omnix.server.observability.StructuredLogger
import com.omnix.server.ratelimit.RateLimitDecision
import com.omnix.server.ratelimit.RateLimiter
...
class LicenseBillingHttpHandler(
    ...
    /** Сырой omx_-токен из заголовка (для одноразовой привязки устройства). */
    private fun bearerLicenseToken(request: HttpRequestContext): String? =
        request.authorizationHeader
            ?.takeIf { it.startsWith("Bearer ", ignoreCase = true) }
            ?.substring("Bearer ".length)?.trim()
            ?.takeIf { it.startsWith("omx_") }
    ...
    private suspend fun checkout(request: HttpRequestContext, fallbackRequestId: String): HttpResponseContext {
        ...
        return when (val result = billingService.createCheckout(
            ...
            is CreateCheckoutOutcome.Success -> HttpResponseContext(
                if (result.order.status == com.omnix.server.billing.BillingOrderStatus.PAID) 200 else 201,
                json.encodeToString(
    ...
    private fun error(
        ...
        json.encodeToString(
            com.omnix.server.api.ApiErrorResponse.serializer(),
