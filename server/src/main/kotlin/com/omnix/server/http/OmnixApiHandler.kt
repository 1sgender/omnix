...
import com.omnix.server.router.RouterResult
...
class OmnixApiHandler(
    ...
    private val entitlementChecker: (com.omnix.server.auth.AuthenticatedClient) -> Boolean = { true },
    ...
    companion object {
        ...
        const val HEADER_DEVICE_ID = "X-Omnix-Device"
    ...
    private suspend fun handleExecute(
        ...
        // ------------------------------------------------- 2. Authentication
        // V007: enforcement-путь требует устройство — omx_-токен сверяется
        // с привязкой к X-Omnix-Device (клиент не источник истины).
        val client = when (
        ...
        if (client.tier !in setOf(
                com.omnix.server.auth.ClientTier.ADMIN,
                com.omnix.server.auth.ClientTier.INTERNAL
