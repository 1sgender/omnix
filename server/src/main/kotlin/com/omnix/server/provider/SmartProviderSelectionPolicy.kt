 ...
 *
 * Схема уже была: AI Gateway ([com.omnix.server.http.OmnixApiHandler]) →
...
class SmartProviderSelectionPolicy(
     ...
     */
    private fun weightsFor(costClass: com.omnix.server.cost.CostClass): Weights = when (costClass) {
        com.omnix.server.cost.CostClass.SIMPLE -> Weights(
            latency = 0.20, reliability = 0.20, rateLimit = 0.10, cost = 0.50
        )
        com.omnix.server.cost.CostClass.MEDIUM -> Weights(
            latency = W_LATENCY, reliability = W_RELIABILITY,
            rateLimit = W_RATE_LIMIT, cost = W_COST
        )
        com.omnix.server.cost.CostClass.HARD -> Weights(
    ...
    companion object {
         ...
         */
        fun costEstimates(
            settings: com.omnix.server.admin.CostSettings
...
class CostPriceSource {
    @Volatile
    private var source: () -> com.omnix.server.admin.CostSettings = {
        com.omnix.server.admin.CostSettings()
    }

    fun set(source: () -> com.omnix.server.admin.CostSettings) {
        this.source = source
    }

    fun get(): com.omnix.server.admin.CostSettings = source()
}
