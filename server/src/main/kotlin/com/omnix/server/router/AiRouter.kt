package com.omnix.server.router

import com.omnix.server.api.AiExecutionRequest
import com.omnix.server.api.ApiErrorCode
import com.omnix.server.api.ApiPrivacyLevel
import com.omnix.server.api.ApiRequestSource
import com.omnix.server.auth.AuthenticatedClient
import com.omnix.server.config.AiGenerationConfig
import com.omnix.server.config.PrivacyPolicyConfig
import com.omnix.server.config.ValidationConfig
import com.omnix.server.license.PlanFeature
import com.omnix.server.license.PlanQuotaGate
import com.omnix.server.observability.LogSanitizer
import com.omnix.server.observability.Metrics
import com.omnix.server.observability.StructuredLogger
import com.omnix.server.provider.ManagerOutcome
import com.omnix.server.provider.ProviderFailureKind
import com.omnix.server.provider.ProviderManager
import com.omnix.server.provider.ProviderRequest
import com.omnix.server.provider.ProviderRequirements
import com.omnix.server.privacy.PrivacyContent
import com.omnix.server.privacy.PromptPrivacyClassifier
import com.omnix.server.privacy.ServerPrivacyClassifier
import com.omnix.server.usage.AiUsageRecord
import com.omnix.server.usage.AsyncUsageTracker
import com.omnix.server.usage.UsageLimitResult
import com.omnix.server.usage.UsageRepository
...
class AiRouter(
    ...
    suspend fun execute(
        ...
        val costClass = com.omnix.server.cost.RequestCostEstimator.classify(
        ...
        logger.info(
            ...
            "approxInputTokens" to com.omnix.server.cost.RequestCostEstimator.approximateInputTokens(
            ...
            history = request.history
                ...
                .mapNotNull { msg ->
                    normalizeRole(msg.role)?.let { role ->
                        com.omnix.server.provider.ProviderMessage(role = role, content = msg.content)
                    }
                },
            requiresWeb = request.requiresWeb,
            // Cost Control: SIMPLE-класс получает урезанный бюджет ответа —
            // короткая реплика не должна платить за длинный вывод.
            maxTokens = com.omnix.server.cost.RequestCostEstimator.outputBudget(
                costClass,
