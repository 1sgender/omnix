...
class AuthConfigRateLimitTest {
    ...
    fun `server config parses tokens with colon in client id and ignores malformed entries`() {
        ...
        val env = mapOf(
            "PORT" to "9090",
            "OMNIX_CLIENT_TOKENS" to " $tokenA:client:a , malformed, :empty, $tokenB:client-b ",
    ...
    fun `invalid security and resource boundaries fail fast`() {
        ...
        assertThrowsIllegalArgument {
            ServerConfig.fromEnv(mapOf("OMNIX_CLIENT_TOKENS" to "weak:client")::get)
        }
        assertThrowsIllegalArgument {
            ServerConfig.fromEnv(
                mapOf("OMNIX_CLIENT_TOKENS" to "${"x".repeat(257)}:client")::get
            )
        }
        assertThrowsIllegalArgument {
            val whitespaceToken = "a".repeat(16) + " " + "b".repeat(16)
            ServerConfig.fromEnv(mapOf("OMNIX_CLIENT_TOKENS" to "$whitespaceToken:client")::get)
        }
        val duplicate = "d".repeat(32)
        assertThrowsIllegalArgument {
            ServerConfig.fromEnv(
                mapOf("OMNIX_CLIENT_TOKENS" to "$duplicate:first,$duplicate:second")::get
            )
        }
        assertThrowsIllegalArgument {
