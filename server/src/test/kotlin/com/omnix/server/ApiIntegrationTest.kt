...
import com.omnix.server.api.AiExecutionResponse
...
import com.omnix.server.provider.AiProvider
import com.omnix.server.provider.DefaultProviderSelectionPolicy
import com.omnix.server.provider.ProviderCapabilities
import com.omnix.server.provider.ProviderFailureKind
import com.omnix.server.provider.ProviderResult
...
import com.omnix.server.privacy.PrivacyClassification
import com.omnix.server.privacy.PrivacyReason
...
class ApiIntegrationTest {
    ...
    private fun harness(
        ...
        privacyClassifier: ServerPrivacyClassifier =
            com.omnix.server.privacy.PromptPrivacyClassifier
    ): Harness {
    ...
    private fun post(
        ...
        path: String = OmnixApiHandler.PATH_EXECUTE
    ) = HttpRequestContext(
        method = "POST",
        path = path,
    ...
    fun `forbidden client cannot access admin metrics`() = runBlocking {
        ...
        val response = h.handler.handle(
            HttpRequestContext("GET", OmnixApiHandler.PATH_ADMIN_METRICS, "Bearer $validToken", "", 0)
        )
    ...
    fun `admin client can access metrics`() = runBlocking {
        ...
        val response = h.handler.handle(
            HttpRequestContext("GET", OmnixApiHandler.PATH_ADMIN_METRICS, "Bearer $adminToken", "", 0)
        )
    ...
    fun `health endpoint is public`() = runBlocking {
        ...
        val response = h.handler.handle(
            HttpRequestContext("GET", OmnixApiHandler.PATH_HEALTH, null, "", 0)
        )
    ...
    fun `known endpoint with wrong method returns 405`() = runBlocking {
        ...
        for ((method, path) in listOf(
            "POST" to OmnixApiHandler.PATH_HEALTH,
            "GET" to OmnixApiHandler.PATH_EXECUTE,
            "POST" to OmnixApiHandler.PATH_ADMIN_METRICS
        )) {
    ...
    fun `UTF-8 byte size is enforced even when supplied content length is wrong`() = runBlocking {
        ...
        val response = h.handler.handle(
            HttpRequestContext("POST", OmnixApiHandler.PATH_EXECUTE, "Bearer $validToken", body, 1)
        )
