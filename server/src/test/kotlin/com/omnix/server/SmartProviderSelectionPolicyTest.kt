...
import com.omnix.server.provider.CostEstimate
...
import com.omnix.server.provider.ProviderPerformanceTracker
import com.omnix.server.provider.ProviderRequest
import com.omnix.server.provider.ProviderRequirements
import com.omnix.server.provider.SmartProviderSelectionPolicy
...
class SmartProviderSelectionPolicyTest {
    ...
    fun `rate limited provider is deprioritized`() {
        ...
        repeat(3) { tracker.recordFailure(ProviderId.GEMINI, com.omnix.server.provider.ProviderFailureKind.RATE_LIMITED) }
        val p = policy(performance = tracker)
    ...
    fun `failing provider is ranked last among measured peers`() {
        ...
        repeat(4) { tracker.recordFailure(ProviderId.OPENROUTER, com.omnix.server.provider.ProviderFailureKind.SERVER_ERROR) }
        val p = policy(performance = tracker)
    ...
    fun `circuit-open provider is never selected regardless of score`() {
        ...
        health.recordFailure(ProviderId.GROQ, com.omnix.server.provider.ProviderFailureKind.SERVER_ERROR, "boom")
    ...
    fun `provider manager records measured stats for smart ranking`() = runBlocking {
        ...
        val manager = ProviderManager(
            providers = listOf(
                FakeAiProvider.failing(ProviderId.GROQ, com.omnix.server.provider.ProviderFailureKind.RATE_LIMITED, 429),
        ...
        assertTrue(outcome is com.omnix.server.provider.ManagerOutcome.Success)
