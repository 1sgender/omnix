...
class AndroidFlavorConfigurationTest {
    ...
    fun `Android environments are explicit and production cannot be overridden`() {
        ...
        val constants = Files.readString(root.resolve(
            "app/src/main/java/com/omnix/assistant/core/constants/AppConstants.kt"
        ))
        ...
        assertTrue(gradle.contains("val productionApiUrl = \"https://api.omnix.ai\""))
        assertFalse(gradle.contains("gradleProperty(\"OMNIX_PROD_API_BASE_URL\")"))
        assertTrue(constants.contains("BuildConfig.OMNIX_API_BASE_URL"))
        assertFalse(constants.contains("const val OMNIX_API_BASE_URL"))
    ...
    fun `release uses R8 resource shrinking and endpoint artifact validation`() {
        ...
        assertTrue(script.contains("staging-api.omnix.ai"))
