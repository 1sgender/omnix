...
class SharedStateArchitectureTest {
    ...
    fun `production composition uses shared security and accounting state`() {
        ...
        val main = Files.readString(root.resolve("server/src/main/kotlin/com/omnix/server/Main.kt"))
        val compose = Files.readString(root.resolve("deploy/docker-compose.production.yml"))
        val deploymentConfig = Files.readString(
            root.resolve("server/src/main/kotlin/com/omnix/server/config/DeploymentSecurityConfig.kt")
        )
    ...
    fun `shared state migration and retention schema remain registered`() {
        ...
        val database = Files.readString(
            root.resolve("server/src/main/kotlin/com/omnix/server/persistence/Database.kt")
        )
