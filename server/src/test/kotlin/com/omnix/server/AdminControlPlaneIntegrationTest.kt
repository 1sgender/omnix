...
import com.omnix.server.admin.AdminAuditQuery
import com.omnix.server.admin.AdminHttpHandler
import com.omnix.server.admin.AdminLoginResult
...
class AdminControlPlaneIntegrationTest : PostgresTestSupport() {
    ...
    fun buildHandler() {
        ...
        handler = AdminHttpHandler(
            auth = com.omnix.server.admin.AdminAuthService(
                accounts = accounts,
                sessions = sessions,
    ...
    private fun get(token: String?, path: String): com.omnix.server.http.HttpResponseContext = runBlocking {
        handler.handle(
            HttpRequestContext(
    ...
    private fun post(token: String?, path: String, body: String = "{}"): com.omnix.server.http.HttpResponseContext = runBlocking {
        handler.handle(
            HttpRequestContext(
    ...
    private fun put(token: String?, path: String, body: String): com.omnix.server.http.HttpResponseContext = runBlocking {
        handler.handle(
            HttpRequestContext(
    ...
    fun `ai settings update applies runtime overrides and validates input`() {
        ...
        assertFalse("GROQ должен быть выключен в рантайме", overrides.enabled(com.omnix.server.provider.ProviderId.GROQ) ?: true)
        assertEquals(1, overrides.priority(com.omnix.server.provider.ProviderId.GROQ))
        ...
        assertFalse(overrides.enabled(com.omnix.server.provider.ProviderId.GROQ) == true)
        ...
        dataSource.connection.use { c ->
            ...
            c.prepareStatement(
                "INSERT INTO licenses (id, code_hash, code_hint, status, billing_status, issued_at, starts_at, expires_at, product_id, plan_id, account_id, " +
                    ...
                    "VALUES (?, decode('aabb', 'hex'), 'AA', 'ACTIVE', 'PAID', ?, now() - interval '1 day', now() + interval '30 days', 'omnix', 'pro_monthly', ?, " +
