...
import com.omnix.server.cost.CostClass
import com.omnix.server.cost.RequestCostEstimator
...
class CostControlTest {
    ...
    private fun select(
        ...
        val all = listOf(
            com.omnix.server.FakeAiProvider.ok(ProviderId.GROQ),
            com.omnix.server.FakeAiProvider.ok(ProviderId.GEMINI)
        )
    ...
    fun `hard class routes to the most reliable provider ignoring price`() {
        ...
        repeat(3) { tracker.recordFailure(ProviderId.GEMINI, com.omnix.server.provider.ProviderFailureKind.SERVER_ERROR) }
        ...
        val all = listOf(
            com.omnix.server.FakeAiProvider.ok(ProviderId.GEMINI),
            com.omnix.server.FakeAiProvider.ok(ProviderId.OPENROUTER)
        )
