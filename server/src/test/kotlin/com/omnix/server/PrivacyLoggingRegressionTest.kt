...
class PrivacyLoggingRegressionTest {
    ...
    fun `confirmed Android plaintext logging bypasses remain removed`() {
        ...
        val voice = read(root, "app/src/main/java/com/omnix/assistant/voice/orchestrator/VoiceInteractionOrchestrator.kt")
        val loop = read(root, "app/src/main/java/com/omnix/assistant/agent/engine/AgentCognitiveLoop.kt")
        val accessibility = read(root, "app/src/main/java/com/omnix/assistant/agent/tools/accessibility/OmnixAccessibilityService.kt")
        val automation = read(root, "app/src/main/java/com/omnix/assistant/agent/automation/engine/PersonalAutomationEngine.kt")
        val localAdapter = read(root, "app/src/main/java/com/omnix/assistant/agent/decision/LocalAiExecutorAdapter.kt")
        val translator = read(root, "app/src/main/java/com/omnix/assistant/agent/translator/LiveTranslatorEngine.kt")
    ...
    fun `server structured logs and usage never include prompt plaintext`() {
        val root = repositoryRoot()
        val router = read(root, "server/src/main/kotlin/com/omnix/server/router/AiRouter.kt")
        val provider = read(root, "server/src/main/kotlin/com/omnix/server/provider/ProviderManager.kt")
