...
import com.omnix.server.billing.BillingProvider
import com.omnix.server.billing.BillingProviderException
...
import com.omnix.server.billing.ProviderCheckout
import com.omnix.server.billing.hmacSha256Hex
...
class LicenseApiIntegrationTest : PostgresTestSupport() {
    ...
    fun buildLicenseHandler() {
        ...
        licenseService.upsertPlan(
            BillingPlan(
                "earclip-monthly", "omnix-clip", "Monthly", 30, 1_400, "USD",
    ...
    fun `admin issuance redeem authenticated validation and checkout reach database`() = runBlocking {
        ...
        assertTrue(redeemed.accessToken.startsWith("omx_"))
    ...
    fun `ambiguous checkout has stable retry safe HTTP contract and signed recovery`() = runBlocking {
        ...
        val webhookBody =
            """{"event_id":"evt_reconcile_api_123","event_type":"transaction.completed","occurred_at":"$occurredAt","data":{"id":"txn_reconciled_1234567890","custom_data":{"omnix_order_id":"${firstCheckout.orderId}"}}}"""
    ...
    fun `unknown and foreign-device replays have indistinguishable public response`() = runBlocking {
        val unknown = handler.handle(
            request(
                ...
                body = """{"code":"OMX-AAAAA-AAAAA-AAAAA-AAAAA","device_id":"device-abcdefgh"}"""
            )
        )!!
