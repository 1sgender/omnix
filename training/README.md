# Фаза 2: кастомная модель «Jarvis»

Day-one в APK едет предобученная `hey_jarvis_v0.1.onnx` (openWakeWord v0.5.1).
Этот каталог — спецификация и инструменты обучения собственной модели
на короткое «Jarvis», когда будет собран датасет. Статус: NOT STARTED.

## Спецификация датасета (минимум для старта обучения)

- ≥500 чистых произнесений «Jarvis» (цель: ≥2000), ≥20 дикторов,
  мужчины/женщины/дети, 16 кГц mono int16.
- Негативы: ≥10 ч фонов (тишина комнат, ТВ, улица, офис) + трудные
  негативы (похожие слова: «service», «jar», «this» — минимум 200 шт).
