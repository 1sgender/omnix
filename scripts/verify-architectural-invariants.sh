...
check "H-02: SendPromptUseCase calls withContextualClassification" \
      ...
      "$APP/java/com/omnix/assistant/domain/usecases/SendPromptUseCase.kt"
check "H-02: AIRepositoryImpl does NOT call classifySafely" \
      "classifySafely" \
      "$APP/java/com/omnix/assistant/data/repository/AIRepositoryAndSettingsImpl.kt" \
      hasnt
check "H-02: OmnixApiAiClient does NOT call classifySafely" \
      "classifySafely" \
      "$APP/java/com/omnix/assistant/ai/OmnixApiAiClient.kt" \
      hasnt
check "H-02: server AiRouter classifier untouched (trust boundary)" \
      "classifySafely" \
      "$SERVER/kotlin/com/omnix/server/router/AiRouter.kt"

# H-03: AutomationScheduleReceiver uses the safe goAsync pattern.
check "H-03: AutomationScheduleReceiver uses SupervisorJob" \
      "SupervisorJob()" \
      "$APP/java/com/omnix/assistant/agent/automation/scheduler/AutomationScheduleReceiver.kt"
...
# H-04: ManualWakeWordTrigger routes through OmnixVoiceService.start,
# startServicePipeline is idempotent.
check "H-04: startServicePipeline has isServiceActive guard" \
      "fun startServicePipeline()" \
      "$APP/java/com/omnix/assistant/voice/orchestrator/VoiceInteractionOrchestrator.kt"
...
# OmnixVoiceService (onServiceConnected → orchestrator.startServicePipeline),
# UI-слой не дёргает пайплайн в обход сервиса.
check "H-04: voice pipeline entry lives in OmnixVoiceService" \
      "startServicePipeline" \
      "$APP/java/com/omnix/assistant/voice/service/OmnixVoiceService.kt"
if grep -rn "startServicePipeline" "$APP/java/com/omnix/assistant/presentation" >/dev/null 2>&1; then
  echo "FAIL: H-04: presentation layer must not start voice pipeline directly (use OmnixVoiceService)"
  echo "       matched: $(grep -rn "startServicePipeline" "$APP/java/com/omnix/assistant/presentation" | head -3)"
  fail=1
else
...
check "H-05: modelOverride removed from AIClient" \
      "modelOverride" \
      "$APP/java/com/omnix/assistant/ai" \
      hasnt
...
check "H-06: OmnixApplication.onCreate does NOT call sendBroadcast" \
      "sendBroadcast" \
      "$APP/java/com/omnix/assistant/OmnixApplication.kt" \
      hasnt
check "H-06: AutomationReconcileWorker uses ExistingWorkPolicy.KEEP" \
      "ExistingWorkPolicy.KEEP" \
      "$APP/java/com/omnix/assistant/agent/automation/scheduler/AutomationReconcileWorker.kt"
...
# M-01: OmnixApiHandler parses body ONCE; size check uses body.length, not toByteArray.
check "M-01: OmnixApiHandler parses JSON exactly once" \
      "decodeFromString(AiExecutionRequest" \
      "$SERVER/kotlin/com/omnix/server/http/OmnixApiHandler.kt"
# Count actual code calls to decodeFromString — not comments.
body=$(grep -n "decodeFromString" "$SERVER/kotlin/com/omnix/server/http/OmnixApiHandler.kt" | grep -v "^[0-9]*:[[:space:]]*//" | wc -l)
if [ "$body" = "1" ]; then
...
check "M-01: OmnixApiHandler measures UTF-8 body bytes for size check" \
...
check "M-02: AsyncUsageTracker has pruneStaleEntries" \
      "pruneStaleEntries" \
      "$SERVER/kotlin/com/omnix/server/usage/AsyncUsageTracker.kt"
check "M-02: AsyncUsageTracker has CLEANUP_INTERVAL_MS" \
      "CLEANUP_INTERVAL_MS" \
      "$SERVER/kotlin/com/omnix/server/usage/AsyncUsageTracker.kt"

# M-10: SlidingWindowRateLimiter only in test sourceset.
if [ -f "$SERVER/kotlin/com/omnix/server/ratelimit/SlidingWindowRateLimiter.kt" ]; then
...
check "M-10: SlidingWindowRateLimiter exists in server/test" \
      ...
      "$SERVER_TEST/kotlin/com/omnix/server/ratelimit/SlidingWindowRateLimiter.kt"
check "M-10: server/main retains RateLimiter interface" \
      "interface RateLimiter" \
      "$SERVER/kotlin/com/omnix/server/ratelimit/RateLimiter.kt"
...
check "VERIFY: FastCommandRouter has no pre-execution success claims" \
      ...
      "$APP/java/com/omnix/assistant/agent/fast/FastCommandRouter.kt" \
      hasnt
check "VERIFY: read-back verification module exists" \
      "object ExecutionVerification" \
      "$APP/java/com/omnix/assistant/agent/tools/verification/ExecutionVerification.kt"
check "VERIFY: SetVolumeTool reads volume back before success" \
      "ExecutionVerification.pollFor" \
      "$APP/java/com/omnix/assistant/agent/tools/device/SetVolumeTool.kt"
check "VERIFY: SetBrightnessTool verifies written brightness" \
      "brightnessVerified" \
      "$APP/java/com/omnix/assistant/agent/tools/device/SetBrightnessTool.kt"
check "VERIFY: DoNotDisturbTool verifies interruption filter" \
      "dndVerified" \
      "$APP/java/com/omnix/assistant/agent/tools/device/DoNotDisturbTool.kt"
check "VERIFY: AlarmTimerTool confirms alarm via nextAlarmClockInfo" \
      "nextAlarmMatchesHour" \
      "$APP/java/com/omnix/assistant/agent/tools/productivity/AlarmTimerTool.kt"

# Единый контракт Tool Registry 2.0: verify() и mapError() — члены OmniTool,
# ToolExecutor обеспечивает фазу Verification для обоих путей выполнения.
check "VERIFY: OmniTool declares verification contract member" \
      "suspend fun verify(arguments: JsonObject, draft: ToolExecutionResult)" \
      "$APP/java/com/omnix/assistant/agent/core/OmniTool.kt"
check "VERIFY: OmniTool declares error mapping contract member" \
      "fun mapError(arguments: JsonObject, error: Throwable)" \
      "$APP/java/com/omnix/assistant/agent/core/OmniTool.kt"
check "VERIFY: OmniTool declares permissions contract member" \
      "val requiredPermissions: List<String>" \
      "$APP/java/com/omnix/assistant/agent/core/OmniTool.kt"
check "VERIFY: ToolExecutor enforces verification step" \
      "tool.verify(call.arguments, draft)" \
      "$APP/java/com/omnix/assistant/agent/executor/ToolExecutor.kt"
...
check "POLICY: ToolPermissionManager consults ActionPolicyEngine" \
      ...
      "$APP/java/com/omnix/assistant/agent/safety/ToolPermissionManager.kt"
check "POLICY: money amount detector exists" \
      "object MoneyAmountDetector" \
      "$APP/java/com/omnix/assistant/agent/policy/MoneyAmountDetector.kt"
check "POLICY: automation origin escalates communications" \
      "origin == ActionOrigin.AUTOMATION" \
      "$APP/java/com/omnix/assistant/agent/policy/ActionPolicyEngine.kt"
check "POLICY: automation engine declares non-bypass origin" \
      "ActionOrigin.AUTOMATION" \
      "$APP/java/com/omnix/assistant/agent/automation/engine/PersonalAutomationEngine.kt"
...
check "A11Y: package policy covers real fintech" \
      ...
      "$APP/java/com/omnix/assistant/agent/tools/accessibility/AccessibilityPrivacyPolicy.kt"
check "A11Y: content sanitizer masks OTP/cards at capture" \
      "ScreenTextSanitizer.sanitize(value)" \
      "$APP/java/com/omnix/assistant/agent/tools/accessibility/OmnixAccessibilityService.kt"
check "A11Y: screen content marker flows through plan summary" \
      "containsScreenContent = screenContentSeen" \
      "$APP/java/com/omnix/assistant/agent/engine/AgentCognitiveLoop.kt"
...
check "A11Y: bypass persistence scrubs screen content" \
      ...
      "$APP/java/com/omnix/assistant/presentation/chat/ChatViewModel.kt"
...
check "LICENSE: AI path enforces token device binding" \
      ...
      "server/src/main/kotlin/com/omnix/server/http/OmnixApiHandler.kt"
check "LICENSE: license authenticator requires device on enforcement path" \
      ...
      "server/src/main/kotlin/com/omnix/server/auth/Auth.kt"
check "LICENSE: validate self-heals legacy token binding" \
      "licenseService.bindTokenDevice" \
      "server/src/main/kotlin/com/omnix/server/http/LicenseBillingHttpHandler.kt"
check "LICENSE: client sends device header on trusted backend" \
      "X-Omnix-Device" \
      "$APP/java/com/omnix/assistant/data/remote/interceptor/AuthInterceptor.kt"
...
check "CLIP: challenges are single-use and expiring" \
      ...
      "server/src/main/kotlin/com/omnix/server/clip/JdbcClipDeviceRepository.kt"
check "CLIP: canonical attestation message is version-domain separated" \
      "JARVIS-CLIP-ATTEST-v1" \
      "server/src/main/kotlin/com/omnix/server/clip/ClipAttestationService.kt"
...
check "CLIP: android verifier is fail-closed pure ECDSA" \
      "getOrDefault(false)" \
      "$APP/java/com/omnix/assistant/core/clip/ClipIdentityVerifier.kt"
check "CLIP: trust store anchored in server responses only" \
      "ТОЛЬКО из ответов сервера" \
      "$APP/java/com/omnix/assistant/core/clip/ClipTrustStore.kt"
check "CLIP: no fake transport success - firmware contract is honest" \
      "ClipTransportResult.Unavailable" \
      "$APP/java/com/omnix/assistant/core/clip/ClipAttestationApi.kt"
...
check "LOCAL-FIRST: translation has an on-device provider" \
      ...
      "$APP/java/com/omnix/assistant/agent/translator/LocalLlmTranslationProvider.kt"
...
check "LOCAL-FIRST: engine prefers offline providers (local-first order)" \
      ...
      "$APP/java/com/omnix/assistant/agent/translator/LiveTranslatorEngine.kt"
...
check "LOCAL-FIRST: device lane routes without any LLM" \
      ...
      "$APP/java/com/omnix/assistant/agent/decision/ExecutionDecisionEngine.kt"

check "LOCAL-FIRST: router metrics singleton exists" \
      "class ExecutionRouterMetrics" \
      "$APP/java/com/omnix/assistant/agent/metrics/ExecutionRouterMetrics.kt"
...
check "SMART-ROUTER: measured performance tracker exists" \
      ...
      "server/src/main/kotlin/com/omnix/server/provider/ProviderPerformanceTracker.kt"
...
check "FALLBACK: default provider attempts bounded (2)" \
      ...
      "server/src/main/kotlin/com/omnix/server/config/ServerConfig.kt"
...
check "FALLBACK: 429 is never retried on the same provider" \
      ...
      "server/src/main/kotlin/com/omnix/server/provider/AiProvider.kt"
...
check "COST-CONTROL: request cost estimator is pure and deterministic" \
      ...
      "server/src/main/kotlin/com/omnix/server/cost/RequestCostEstimator.kt"
check "COST-CONTROL: cost class flows through provider requirements" \
      "costClass: com.omnix.server.cost.CostClass" \
      "server/src/main/kotlin/com/omnix/server/provider/ProviderManager.kt"
...
check "COST-CONTROL: router estimates cost before provider call" \
      ...
      "server/src/main/kotlin/com/omnix/server/router/AiRouter.kt"
...
check "VOICE-LATENCY: percentile store exists" \
      ...
      "$APP/java/com/omnix/assistant/agent/metrics/VoiceLatencyMetrics.kt"
...
check "BATTERY: idle-unload scheduler exists (noteUsed + onIdle)" \
      "fun noteUsed()" \
      "$APP/java/com/omnix/assistant/agent/localai/mediapipe/IdleUnloadScheduler.kt"
check "BATTERY: every model use extends the idle window" \
      "idleUnloadScheduler.noteUsed()" \
      "$APP/java/com/omnix/assistant/agent/localai/mediapipe/MediaPipeModelManager.kt"
...
check "BATTERY: scheduler covered by virtual-time tests" \
      "advanceTimeBy" \
      "$APP_TEST/java/com/omnix/assistant/agent/localai/mediapipe/IdleUnloadSchedulerTest.kt"
...
check "EAR-MODE: no-headset guard in routeAudioToEarbud (no comm-mode without earbud)" \
      ...
      "$APP/java/com/omnix/assistant/voice/audio/BluetoothAudioRouter.kt"
...
check "EAR-MODE: interpreter screen takes mic from wake-word" \
      "wakeWordEngine.stop()" \
      "$APP/java/com/omnix/assistant/presentation/translator/LiveInterpreterViewModel.kt"
...
check "EAR-MODE: TTS requests transient may-duck audio focus" \
      ...
      "$APP/java/com/omnix/assistant/voice/tts/TextToSpeechManager.kt"
check "EAR-MODE: TTS focus policy pinned by test" \
      "permanent loss stops speech" \
      "$APP_TEST/java/com/omnix/assistant/voice/tts/TextToSpeechFocusPolicyTest.kt"
...
check "MEMORY: ExecutionRequest carries bounded memoryContext" \
      ...
      "$APP/java/com/omnix/assistant/agent/decision/ExecutionModels.kt"
...
check "MEMORY: cloud prompt gets retrieval block only (never full memory)" \
      ...
      "$APP/java/com/omnix/assistant/agent/decision/ExecutionAdapters.kt"
check "MEMORY: local prompt gets retrieval block (offline recall)" \
      "request.memoryContext" \
      "$APP/java/com/omnix/assistant/agent/localai/OmniLocalPromptBuilder.kt"
check "MEMORY: memory injection pinned by tests" \
      "private fact stored in memory blocks cloud without consent" \
      "$APP_TEST/java/com/omnix/assistant/memory/MemoryRetrievalInjectionTest.kt"
...
check "AGENT-CORE: reroute pinned by test" \
      ...
      "$APP_TEST/java/com/omnix/assistant/agent/decision/ExecutionDecisionEngineTest.kt"
...
check "ADMIN: licenses status filter in queries (bind-param, no string concat)" \
      ...
      "$SERVER/kotlin/com/omnix/server/admin/AdminQueries.kt"
...
check "OBSERVABILITY: omx ULID generator exists (Crockford base32)" \
      ...
      "$APP/java/com/omnix/assistant/core/request/RequestIds.kt"
...
check "OBSERVABILITY: client no longer generates a disconnected UUID per call" \
      ...
      "$APP/java/com/omnix/assistant/data/remote/OmnixApiClient.kt"
...
check "V03-STAB: regression test for chase still pins the block" \
      ...
      "$APP_TEST/java/com/omnix/assistant/agent/tools/accessibility/AccessibilityPrivacyPolicyTest.kt"
