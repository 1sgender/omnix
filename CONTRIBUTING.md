...
## CI-триггеры (path-фильтры)

- `push`-прогоны гоняются **только для main** и только для не-докс
  изменений: `**/*.md`, `docs/**`, `LICENSE`, `NOTICE` игнорируются
  (docs-only мерж не тратит ~15 мин Actions на полный пайплайн ×2);
- `pull_request` — **всегда полный пайплайн без фильтров**: required-чеки
  обязаны прийти на каждый PR, иначе мерж зависнет. Фильтровать
  `pull_request` по путям нельзя, пока чеки числятся required;
- feature-ветки не получают push-прогонов — их полностью дублирует
  pull_request-прогон того же SHA;
- авто-staging релиз (release.yml) режется после зелёного Build в main
  для не-докс мержей; `schedule`-скан и `workflow_dispatch` не фильтруются.
